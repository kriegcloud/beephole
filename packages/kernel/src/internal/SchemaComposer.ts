 ...
 * @since 0.1.0
 */
import { Data, Effect, Option, SchemaAST, pipe } from "effect";
 ...
 * @category models
 */
type SchemaType<F extends S.Struct.Fields> = S.Schema.Type<S.Struct<F>>;
 ...
 * @category models
 */
export interface BuilderOp<F extends S.Struct.Fields, R> {
  readonly schema: S.Struct<F>;
 ...
 * @category models
 */
export type Builder<F extends S.Struct.Fields, R> = BuilderOp<F, R> &
...
export const createBuilderLenses = <A>(
  schema: S.Struct<AYS.AYS>,
): BuilderLens<A> =>
  pipe(
    R.toEntries(schema.fields),
 ...
 * @category constructors
 */
const getSchemaDefaults = <A, F extends S.Struct.Fields>(
  schema: S.Struct<F>,
): Partial<A> => {
 ...
 * // {
 */
export const define = <F extends S.Struct.Fields>(
  schema: S.Struct<F>,
  defaults?: Partial<SchemaType<F>>,
): Builder<F, S.Schema.Context<F[keyof F]>> => {
 ...
 * import { define, compose } from "@e2/domain/primitives"
 *
 * const UserSchema = S.Struct({
 *   name: S.String,
 *   age: S.Number,
 *   roles: S.Array(S.String).annotations({ default: ["user"] })
 * })
 *
 * const User = define(UserSchema)
 *
